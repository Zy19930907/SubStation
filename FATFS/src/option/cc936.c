...
void ReadCC936(void)
{
	u16 i;
	u16 cnt,remain;
	uni2oem = mymalloc(SRAMEX,87172);
	mf_open("0:/FATFSFONT/uni2oem.c",FA_OPEN_EXISTING | FA_READ);
	cnt = 87172 / 2048;
	remain = 87172 % 2048;
	for(i=0;i<cnt;i++)
		mf_read((u8*)(uni2oem + 2048 * i),2048);
	mf_read((u8*)(uni2oem + 2048 * i),remain);
	mf_close();
	
	oem2uni = mymalloc(SRAMEX,87156);
	mf_open("0:/FATFSFONT/oem2uni.c",FA_OPEN_EXISTING | FA_READ);
	cnt = 87156 / 2048;
	remain = 87156 % 2048;
	for(i=0;i<cnt;i++)
		mf_read((u8*)(oem2uni + 2048 * i),2048);
	mf_read((u8*)(oem2uni + 2048 * i),remain);
	mf_close();
}
