...
void WT2031Init(void)
{
	...
	//SHDN
	SHDN_PORT->MODER &= ~((u32)0x03 << (SHDN_PIN << 1));//清除原有配置
	SHDN_PORT->MODER |=  ((u32)0x01 << (SHDN_PIN << 1));//通用输出模式
	SHDN_PORT->OSPEEDR |= ((u32)0x03 << (SHDN_PIN << 1));//80MHz
	SHDN_PORT->PUPDR |= ((u32)0x01 << (SHDN_PIN << 1));//上拉
	SHDN_PORT->ODR |= (1<<SHDN_PIN);
//SHDN_PORT->ODR &= ~(1<<SHDN_PIN);
	
	//MUTE
	MUTE_PORT->MODER &= ~((u32)0x03 << (MUTE_PIN << 1));//清除原有配置
	MUTE_PORT->MODER |=  ((u32)0x01 << (MUTE_PIN << 1));//通用输出模式
	MUTE_PORT->OSPEEDR |= ((u32)0x03 << (MUTE_PIN << 1));//80MHz
	MUTE_PORT->PUPDR |= ((u32)0x01 << (MUTE_PIN << 1));//上拉
	MUTE_PORT->ODR |= (1<<MUTE_PIN);
	...
	//RX_CLK
	WT2031_RX_CLKPROT->MODER &= ~(0x03 << (WT2031_RX_CLKPIN << 1));//RX_CLK输出模式
	...
	//RX_STRB
	WT2031_RX_STRBPROT->MODER &= ~(0x03 << (WT2031_RX_STRBPIN << 1));//RX_CLK输出模式
	...
	//RX_DATA
	WT2031_RX_DATAPROT->MODER &= ~(0x03 << (WT2031_RX_DATAPIN << 1));//RX_CLK输出模式
	...
	//TX_CLK
	WT2031_TX_CLKPROT->MODER &= ~(0x03 << (WT2031_TX_CLKPIN << 1));//RX_CLK输出模式
	...
	//TX_STRB
	WT2031_TX_STRBPROT->MODER &= ~(0x03 << (WT2031_TX_STRBPIN << 1));//RX_CLK输出模式
	...
	//TX_DATA
	WT2031_TX_DATAPROT->MODER &= ~(0x03 << (WT2031_TX_DATAPIN << 1));//RX_CLK输出模式
