...
{  
	...
	MY_NVIC_SetVectorTable(STARTADDR,0x0);
}		    


void IoInit(GPIO_TypeDef* Portx,u8 Pinx,u8 mode)
{
	Portx->MODER &= ~(0x03 << (Pinx << 1));
	Portx->OTYPER &= ~(0x01 << Pinx);
	if(mode)
	{
		Portx->MODER |= (0x01 << (Pinx << 1));
		Portx->OSPEEDR |= (0x03 << (Pinx << 1));
//		Portx->PUPDR |= (0x01 << (Pinx << 1));
	}
	else
	{
		Portx->PUPDR &= ~(0x01 << (Pinx << 1));
...
void SetIoValue(GPIO_TypeDef* Portx,u8 Pinx,u32 val)
{
	if(val)
		Portx->ODR |= (0x01 << Pinx);
	else
...
u8 GetIoValue(GPIO_TypeDef* Portx,u8 Pinx)
{	
	return ((Portx->IDR >> Pinx) & 0x01);
}
